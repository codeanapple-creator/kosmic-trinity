...
export function useTransparentLogo(src: string): string {
  ...
  useEffect(() => {
    const img = new Image();
    // Do NOT set crossOrigin for same-origin Vite-bundled assets.
    // Setting crossOrigin="anonymous" on an asset served without CORS headers
    // causes Android WebViews (including Instagram's in-app browser) to stall
    // the request indefinitely — the onload/onerror events never fire, leaving
    // a pending network request that keeps the browser loading indicator spinning.
    img.onload = () => {
      try {
        const transparent = removeBackground(img);
        setProcessedSrc(transparent);
      } catch {
        // Canvas security error (tainted canvas) — fall back to original src
        setProcessedSrc(src);
      }
    };
